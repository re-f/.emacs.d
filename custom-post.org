...
#+RESULTS:
: ((TODO :foreground white) (WAITING :background gray :foreground white) (NEXT :foreground blue :weight bold) (WAITING . warning) (HANGUP :foreground magenta :weight bold))
...
- 内置的中文输入法
   [[https://github.com/tumashu/pyim]]，使用内置的中文输入法的原因：频繁切换中英文输入法，打破了使用时行云流水的感觉，使用内置的中文输入法，可以做到，中文输入时同时使用 emacs 的快捷键，而不用平凡进行中英文切换
  #+begin_src emacs-lisp :tangle yes
