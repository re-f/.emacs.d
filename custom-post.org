...
**** 编辑习惯
- 模板配置
  理论来说，应该使用 =(setq yas-snippet-dirs (append yas-snippet-dirs (concat org-directory "/config/snippets")))= 但是这条命令的执行结果是 =("~/emacs.d/snippets" . "/Users/monster/Dropbox/knowledgebase-blog/content-org/config/snippets")= 中间总是多了一个“.”， 目前来说只能hardcode了
#+begin_src emacs-lisp :tangle yes
(setq yas-snippet-dirs (append yas-snippet-dirs '("~/Dropbox/knowledgebase-blog/content-org/config/snippet")))
#+end_src
*** 追踪公司项目进展
:PROPERTIES:
...
(setq pyim-punctuation-translate-p '(no auto yes)) ;; 默认使用半角符号
#+end_src
