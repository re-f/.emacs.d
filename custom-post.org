...
#+begin_src emacs-lisp :tangle yes :results silent
  ...
  (defun ref/org-roam-node-annotation (node)
    "Simple annotation with format: < [parent-info] with headline face."
    (let* ((level (org-roam-node-level node))
           (olp (org-roam-node-olp node))
           (parent-title (when (and (> level 0) olp)
                           (car (last olp 1)))))
      (when parent-title
        (concat "< ["
                (propertize parent-title 'face 'org-level-1)
                "]"))))

  (setq org-roam-node-annotation-function #'ref/org-roam-node-annotation)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  ...
  (defun xah-open-in-cursor ()
    "Open current file or dir in cursor.
    ...
    (let ((xpath (if buffer-file-name buffer-file-name (expand-file-name default-directory))))
      ...
      (cond
       ((string-equal system-type "darwin")
        (shell-command (format "open -a Cursor.app %s" (shell-quote-argument xpath))))
       ((string-equal system-type "windows-nt")
        (shell-command (format "cursor.cmd %s" (shell-quote-argument xpath))))
       ((string-equal system-type "gnu/linux")
        (shell-command (format "cursor %s" (shell-quote-argument xpath)))))))
