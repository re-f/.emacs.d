...
需要自己下载的 package 有
#+begin_src bash
git clone git@github.com:kanglmf/emacs-chinese-word-segmentation.git
org-bars.el #chinese-word-segmentation  
#+end_src
*** 在 org-mode 中写代码
...
目标：创建一个 static 文件夹，用于存放附件，并确保在 =org-directory= 下的任意层级的 org 文件中引用 static 中的文件时，都是以 org-directory 为基准的相对路径。此举的好处在于，无论 org 文件本身如何移动，只要引用的是 static/ 目录下的文件，引用就能被正确解析。
...
#+begin_src emacs-lisp :tangle yes :results none
  (use-package gptel
    ...
    (setq gptel-backend
          (gptel-make-openai
              "pumpkinaigc"                    ;; 后端名称，可以任意命名
            :host "api.pumpkinaigc.online"
            :key "sk-neysidbiji2EKQQV88D450CcB0804506Ba8aD9B443839218"
            :models '(gpt-4o)))
    ...
    (setq gptel-backend
            (gptel-make-gemini "Gemini"
              :key "AIzaSyBfMej5vNJh9uaqW1fP97se0JRmpeZUZdE"
              :stream t)
            ;; 设置默认模型
    ...
    (gptel-make-gpt4all "GPT4All"           ;Name of your choosing
      :protocol "http"
      :host "https://api.pumpkinaigc.online/v1"     ;Where it's running
      :models '(o1-mini
                o1-preview
                gpt-4
                gpt-4-turbo
