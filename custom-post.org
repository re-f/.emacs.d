...
#+begin_src emacs-lisp :tangle yes
  ...
  示例:
    ...
    (let* ((month (format-time-string "%b")) ; 当前月份的英文缩写，例如 "Sep"
      ...
      ;; 返回 org-capture 的目标位置，指向当前周的 headline
      (list 'file+headline file headline-week)))

  ;; 添加 org-capture 模板
  (push '("w" "PLAN: 每周工作记录"
          entry  ;; 使用 'entry' 以添加标题
          (file+function "./worklog/2025.org"
                         (lambda () (org-capture-to-year-month-week-worklog (concat org-directory "/worklog/2025.org"))))
          "\n\n*** %?"  ;; 使用三个星号表示子标题，根据需要调整
          :tree-type week)
